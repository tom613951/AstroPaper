...
通过分析 .pdb 调试符号 and 类型引用关系，定位了四个核心 DLL：
...
| B. 替换 RSA 公钥 | 替换 k 中的公钥 + 运行代改变解密 | 复杂，需同时处理加密和解密 |
...
```powershell
$WORK_DIR = "C:\Users\26503\.qoderworkcn\workspace\mrkeavpv1hke4qm4"
while ($true) {
    $monitorProcess = Start-Process -FilePath "node" `
        -ArgumentList "$WORK_DIR\hook_monitor.js" `
...
2. PowerShell 启动 `hook_monitor.js`（Node.js 后台进程）
3. `hook_monitor.js` 每 5 秒轮询 CDP 端口 8087
4. 检测到 CefSharp 就绪后注入 Hook
5. 持续监控 page ID 变化，自动重注入
6. 若 Node 进程崩溃，PowerShell 循环自动重启

### 9.4 早期启动脚本（完整版）

早期版本 `shadowbot_hook_startup.ps1` 包含 SSH 隧道建立（已废弃）和更复杂的逻辑。最终简化为仅运行 `hook_monitor.js` + 崩溃自动重启。
...
| 文件 | 用途 | 说明 |
|------|------|------|
| `ai_hook.js` | 主 Hook 脚本 | 注入到 CefSharp 浏览器的核心代码，含 Provider 配置、System Prompt、工具定义、JSON.stringify/fetch Hook |
| `inject_hook_auto.js` | 动态注入器 | 通过 CDP WebSocket 自动发现页面并注入 Hook |
| `hook_monitor.js` | 后台监控 | 每 5 秒轮询 CDP，检测 page ID 变化自动重注入 |
| `encode_b64.js` | Base64 编码器 | 使用 Node.js Buffer 对 ai_hook.js 进行 UTF-8 安全的 base64 编码 |
| `clear_hook.js` | 清除旧 Hook | 通过 CDP 清除 `__aiHookInstalled` 标志，允许重新注入 |
| `check_hook_status.js` | 状态检查 | 通过 CDP 验证 Hook 是否正常运行 |
| `shadowbot_hook_startup.ps1` | Windows 启动脚本 | 被注册表 Run 键调用，启动并守护 hook_monitor.js |
| `add_startup.reg` | 注册表导入文件 | 将启动脚本注册到 HKCU Run 键，无需管理员权限 |
| `ai_hook_b64.txt` | 编码后的 Hook | base64 编码的 ai_hook.js，供注入器读取 |
| `gemini_proxy.js` | Linux 代理（已废弃） | 原服务器代理，含 mihomo CONNECT 隧道 and 429 重试 |
...
# 3. 注入 Hook
node inject_hook_auto.js

# 4. 验证
node check_hook_status.js
```
...
正常打开影刀即可。Hook 会在 CefSharp 就绪后自动注入。

### 12.4 运行时控制

在影刀的 CefSharp 浏览器控制台（通过 CDP）执行：

```javascript
window.__aiHook.getStatus()              // 查看当前状态
window.__aiHook.switchProvider('gemini_flash')  // 切换到 Gemini Flash
window.__aiHook.setModel('gemini-3.5-flash')    // 切换模型
...
修改 `ai_hook.js` 后需要重新编码和注入：

```bash
node encode_b64.js          # 重新编码
node clear_hook.js          # 清除旧 Hook 标志
node inject_hook_auto.js    # 重新注入
```
...
# 停止后台监控
# 在任务管理器中结束 node.exe (hook_monitor.js) 进程
```
...
| 6 | ERR_HTTP_HEADERS_SENT 崩溃 | 超时后尝试发送错误 pero headers 已发出 | 添加 res.headersSent 检查 |
...
| 11 | Hook 重注入不生效 | __aiHookInstalled 标志阻止重装 | 创建 clear_hook.js 先清除标志 |
| 12 | 新建流程后 Hook 丢失 | CefSharp 页面刷新，page ID 变化 | hook_monitor.js 后台轮询自动重注入 |
...
> `reasoning_trace` 为中文推理过程，描述从上一步到决定调用此工具的完整思考链。虽然 schema 的 `required` 数组中不包含它，但描述制造为 `中文必填`，实际运行时必须填写。
